        ...
        <div id="search_cadet" class="row my-2" style="display:none">
            <div class="col-lg-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body bg-custom3">
                        <form class="user" role="form" method="post" enctype="multipart/form-data" id="save_form" action="<?= base_url(); ?>HOUGP/save_cadet_warning">
                            <div class="form-group row">
                                <div class="col-sm-2">
                                    <h6>&nbsp;Name:</h6>
                                </div>
...
<script>
    ...
    $('#promote_all_btn_term3').on('click', function() {
        ...
        if (validate == 0) {
            ...
            $.ajax({
                url: '<?= base_url(); ?>HOUGP/update_cadet_to_midshipman',
                method: 'POST',
                data: {
                    'p_id': 0,
                    'curr_term': curr_term,
    ...
    $('#promote_all_btn_term4').on('click', function() {
        ...
        if (validate == 0) {
            ...
            $.ajax({
                url: '<?= base_url(); ?>HOUGP/update_cadet_to_sub_lieutenant',
                method: 'POST',
                data: {
                    'p_id': 0,
                    'curr_term': curr_term,
    ...
    $('#select_branches').on('change', function() {
        ...
        $.ajax({
            url: '<?= base_url(); ?>HOUGP/get_semester_list',
            method: 'POST',
            data: {
                'branch_id': branch
            },
    ...
    $("#show_semester_list").on('change', function() {
        ...
        var semester = $(this).val();
        $.ajax({
            url: '<?= base_url(); ?>HOUGP/search_all_cadets_by_term',
            method: 'POST',
            data: {
                'term': term,
                'branch_id': branch,
